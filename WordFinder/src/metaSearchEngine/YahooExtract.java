...
public class YahooExtract {
	...
	public static void main(String[] args) {
		...
		Rule rule = new Rule("", "任何语言", "+十四所 +雷达", "南京十四所", "航母 中国", "", "", "",
				"其他", false,"");
		YahooExtract ye = new YahooExtract(false);

		String url = ye.getUrlByRule(rule, "30");

		System.out.println(url);

		/*
		 * ye.getNextPages(url); for(int i=0;i<ye.nextPages.size();i++){
		 * System.out.println(ye.nextPages.get(i)); }
		 */
	...
	public YahooExtract(boolean isAutoSearch) {
	...
	public int doSearch(String url) {
		...
		try {
			...
			//e.printStackTrace();
			System.err.println("使用雅虎搜索引擎时出现异常");
			if (table_list != null)
				table_list = null;
			if (table_it != null)
				table_it = null;
	...
	public String getUrlByRule(Rule rule, String Num) {
		...
		String url = "http://www.yahoo.cn/s?p=" + q1 + q2 + q4 + site
				+ "&v=web";
		return url;
