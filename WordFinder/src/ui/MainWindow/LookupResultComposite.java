...
import searchUtility.LookupResultThread;

public class LookupResultComposite extends Composite {

	private Text text_beginTime, text_endTime;
	private Combo combo_rule;
	
	public LookupResultComposite(Composite parent) {
	...
	private void createContent() {
		...
		MainWindow.lookupResultFolder = new CTabFolder(this, SWT.BORDER | SWT.CLOSE);
		MainWindow.lookupResultFolder.setLayoutData(new GridData(GridData.FILL_BOTH));
		MainWindow.lookupResultFolder.setMinimumCharacters(10);
		MainWindow.lookupResultFolder.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(final SelectionEvent e) {
				combo_rule.select(combo_rule.indexOf(MainWindow.lookupResultFolder.getSelection().getText().replace("元搜索规则：", "")));
			}
		});
	}
	...
	private void onSearch(){
		...
		new LookupResultThread(ruleName, begin, end).start();
	}
}
